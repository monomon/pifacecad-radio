...
STATIONS = [
    {'name': "Bassdrive",
     'source': 'http://bassdrive.org/v2/streams/BassDrive.pls',
     'info': None},
    {'name': "Party vibe radio",
     'source': 'http://www.partyviberadio.com:8004/listen.pls?sid=1&t=.pls',
     'info': None},
    {'name': "Undergroundfm",
     'source': 'http://uk1.internet-radio.com:8093/listen.pls&t=.pls',
     'info': None},
    {'name': "Future pressure radio",
     'source': 'http://uk3.internet-radio.com:8108/listen.pls&t=.pls',
     'info': None},
]
...
class Radio(object):
        ...
        if self.playing_process:
            for line in self.playing_process.stdout:
                if line and line != '':
                    ...
                    self.update_ps_display(line)
                else:
                    break

    def update_ps_display(self, line):
        self.cad.lcd.set_cursor(0, 1)
        print(line.ljust(LCD_WIDTH - 1))
        self.cad.lcd.write(line.ljust(LCD_WIDTH - 1)[0: LCD_WIDTH])
    ...
    def update_display(self):
        ...
        # update the bottom line with the process's stdout
        # dunno if this is safe on a separate thread...
        self.thread = Thread(target=self.ps_reader)
        self.thread.start()
